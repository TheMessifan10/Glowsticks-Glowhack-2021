...
class Giveaway(commands.Cog):
    ...
    async def gstart(self, ctx):
        ...
        for i, question in enumerate(questionList):
            ...
            if not answer:
                embed = embed(
                  ...
                  description = "You failed to answer the questions"
                )
        ...
        giveawayEmbed.add_field(name="Author:", value=f"{ctx.author}")
        ...
        if len(users) == 0:
            embed = discord.Embed(
              title="Giveaway Error",
              description="There Was No winner Decided"
            )
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
          ...
          description = f"Congrats {winner.mention} has won {answers[2]}"
        )
